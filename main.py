...
template = """
 You are a marketing copywriter with 20 years of experience. You are analyzing customer's background to write personalized product description that only this customer will receive; 
    ...
    CUSTOMER field of work: {fieldofwork};
    TASK: Write a product description that is tailored into this customer's Age group and field of work. Use age group specific slang.;
...
prompt = PromptTemplate(
    input_variables=["agegroup", "fieldofwork", "content"],
    template=template,
)
...
with col1:
    st.markdown("Otstarve: tootetutvustustekstide personaliseerimine igale kliendile või kliendigruppidele; väljundtekst on kohandatud kliendi a) vanuserühmaga ja b) töövaldkonnaga; sisendtekstiks on neutraalses vormis tootekirjeldus. \
    \n\n Kasutusjuhend: 1) valmista ette tootekirjeldus (sisendtekst). 2) määra tarbijasegemendid lähtuvalt vanuserühma ja töövaldkonna kombinatsioonidest. 3) sisesta ükshaaval tarbijasegmentide lõikes eeltoodud info äpi kasutajaliideses, saada ära. \
...
def get_fieldofwork():
    input_text = st.text_input(label="Customers main field of work", key="fieldofwork_input")
    return input_text

fieldofwork_input = get_fieldofwork()
...
#        st.stop()
    ...
    prompt_with_content = prompt.format(agegroup=option_agegroup, fieldofwork=fieldofwork_input, content=content_input)
